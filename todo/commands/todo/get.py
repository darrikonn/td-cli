...
class Get(Command):
    def run(self, id):
        try:
            ...
            RenderOutput(
                "{subsequent_indent}{bold}{blue}{group_name}{reset} ({completed})\n"
            ).render(
                group_name=todo[1] or "global",
                subsequent_indent=" " * 8,
            ...
            RenderOutputWithTextwrap(
                "\n{grey}{bold}{todo_id}{normal}: ", "{details}"
            ).render(details=todo[2], todo_id=todo[0], subsequent_indent=" " * 7)
        except Error as e:
